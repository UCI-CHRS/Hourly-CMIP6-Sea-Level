...
def show_point(df1_x, df1_y, df2_x, df2_y, ax, thresh_y=0.95):
    ...
    # distribution
    ax.plot(df1_x, df1_y, linestyle='solid', c='red', lw=2, label='Point')
    ax.plot(df2_x, df2_y, linestyle='solid', c='black', lw=2, label='Grid')
    # marker
...
def plot_cdf(station_id, ax, x, y):
    twl_files = [
        f for f in Path(
    ...
    twl_files_upsampled = [
        f for f in Path(
            "/media/annika/Annika/cmip6-hourly-sea-level-data/upsampleddata/grd001/obs_ocean_noaa"
        ).glob(f"*hourly*.nc")
        if int(str(f)[-7:-3]) <= 2020
    ]
    df_point = pd.concat([
        df['value']
        for f in twl_files
    ...
    ds_grid = [
        xr.open_dataset(f).sel(x=x, y=y, method="nearest")
        for f in twl_files_upsampled
    ]
    df_grid = pd.concat([x['twl'].to_dataframe() for x in ds_grid])['twl'].sort_index().dropna()
    p1 = stats.ecdf(df_point.dropna())
    p2 = stats.ecdf(df_grid.dropna())
    ax = show_point(p1['value'], p1['p'], p2['value'],
                    p2['p'], ax, thresh_y=0.95)
    ax.set_xlabel("Hourly Total Water Level (m)")
